...
class UserMonsterService:
    ...
    async def get_user_list(
        ...
        results = self.user_monster_repo.get_all_with_stats(
            user_id=user_id, skip=skip, limit=size, sort_by=sort_by, sort_order=sort_order
        )
        total_elements = self.monster_repo.count_all()
        ...
        monster_list = [
            ...
            ) for m, avg, is_drunk in results
        ]

        return Result.success(
            data=Pageable.create(monster_list, total_elements, page, size),
            message="User monster list retrieved successfully",
            status_code=200
        )
