...
class MonsterService:
    ...
    async def get_drunk_list(
        ...
        all_results = self.monster_repository.get_all_owned_stats(user_id=user_id)
    ...
    async def update_drunk_interaction(self, user_id: uuid.UUID, req: DrunkMonsterEntryReq) -> Result[None]:
        if not self.monster_repository.get_by_id(req.monster_id):
            return Result.failure("Monster not found", 404)

        entry = self.monster_repository.get_user_entry(user_id, req.monster_id)
        ...
        self.monster_repository.save_entry(entry)
